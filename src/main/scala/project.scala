...
object Project extends App {
	val spConfig = (new SparkConf)
		...
		*/
		val inputFile = "src/main/resources/data/enron.json"
...
// PART 1
		...
		// select specified fields (SELECT)
		val from = emails.select("sender")
		from.show()

		val to = emails.select("to")
		to.show()

		// val fromTo = emails.select(emails("sender"), emails("to")) // another possibility
		val fromTo = emails.select("sender", "to")
		fromTo.show()

		// select rows meeting a criteria (FILTER)
		// select emails where sender is sfsiren@yahoo.com
		val sfsirenEmails = fromTo.filter(fromTo("sender").equalTo("sfsiren@yahoo.com"))
		sfsirenEmails.show()
		sfsirenEmails.count()	// return number of emails sent by sfsiren

		// select emails where sender is sfsiren@yahoo.com
		val rosaleeEmails = fromTo.filter(fromTo("sender").equalTo("rosalee.fleming@enron.com"))
		rosaleeEmails.show()
		rosaleeEmails.count()	// return number of emails sent by rosalee

		// group together based on a single field. Needs to be followed by an aggregation (e.g. count, min, max, mean, etc)
		// GROUPBY
		val grouped = fromTo.groupBy("sender").count()
		grouped.show(500)
